...
export default class World extends EventEmitter {
	...
	construct() {
		if (this.app.camera.instance) {
			...
			// ADD TO SCENE
			this.scene.add(CURVE_OBJECT, this.cameraLookAtPointIndicator);
			...
			?.add(
				{
					fn: () => {
						const _POS_LOOK_AT = new THREE.Vector3().copy(
							this.isometricRoom?.roomBoard?.position ?? new THREE.Vector3()
						);
			...
			?.add(
				{
					fn: () => {
						const _POS_LOOK_AT = new THREE.Vector3().copy(
							this.isometricRoom?.roomShelves?.position ?? new THREE.Vector3()
						);
			...
			?.add(
				{
					fn: () => {
						const _POS_LOOK_AT = new THREE.Vector3().copy(
							this.isometricRoom?.pcScreen?.position ?? new THREE.Vector3()
						);
